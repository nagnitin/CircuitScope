...
@dataclass
class PronounPrompt:
    """A single pronoun resolution prompt pair (clean + corrupted)."""
    prompt: str
...
class PronounDataset:
    ...
    def generate(self) -> "PronounDataset":
        ...
        for i in range(self.n_prompts):
            ...
            if i % 2 == 0:
                ...
                distractors = [n for n in all_female if n != speaker]
                distractor = self._rng.choice(distractors)
            else:
                speaker = self._rng.choice(all_male)
                recipient = self._rng.choice(all_female)
                distractors = [n for n in all_male if n != speaker]
                distractor = self._rng.choice(distractors)
    ...
    def get_corrupted_prompts(self) -> list[str]:
        """Return list of corrupted prompt strings."""
        return [p.corrupted_prompt for p in self.prompts]
